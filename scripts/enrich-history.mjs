...
function compactCandidate(c) {
  return {
    ...
    lateConfirmationRisk: c.lateConfirmationRisk ?? null,
    priceResponseWeak: c.priceResponseWeak ?? null,
    exhaustionRisk: c.exhaustionRisk ?? null,
    comfortState: c.comfortState ?? null,
    comfortFlags: c.comfortFlags ?? [],
...
const syncEntryRunnerFields = (entries, enrichedPool) =>
  (entries || []).map(entry => {
    ...
    const {
      ...
      lateConfirmationRisk, priceResponseWeak, exhaustionRisk, comfortState, comfortFlags,
